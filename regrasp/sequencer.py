...
class SequenceMaster(object):
    def __init__(self,
        ...
        if DATA_DIR.joinpath("regrasp_graph", f"graph_data_{handover_data.obj_name}.pkl").exists():
            ...
            self.buildGraph(armname="rgt_arm", handover_data=handover_data)  # add the handover grasps of the rgt arm
            self.buildGraph(armname="lft_arm", handover_data=handover_data)  # add the handover grasps of the left arm
    ...
    def loadhandovergrasp(self):
        self.grasp_list_rgt, self.grasp_list_lft, self.fp_list, self.identity_grasp_pairs, self.grasp_list_fp_rgt, self.grasp_list_fp_lft, \
        self.ikfid_fp_gid_rgt, self.ikfid_fp_gid_lft, self.ikjnts_fp_gid_rgt, self.ikjnts_fp_gid_lft = self.hmstr.gethandover()
        self.hndover_pos = [pos for pos in self.fp_list]
    ...
    def buildGraph(self, armname: Literal['rgt_arm', 'lft_arm'], handover_data: HandoverData):
        if armname == "rgt_arm":
        ...
        for fpid in range(len(handover_data.floating_pose_list)):  # iterate the handover positions
            ...
            for pair_id, (rgt_id, lft_id) in enumerate(
                ...
                # manipulability
                self.rbt.fk(component_name=armname, jnt_values=fp_jnts_approach)
                manipulability = self.rbt.manipulability(component_name=armname)
                self.regg.add_node('ho' + armname + str(fp_gid) + 'pos' + str(fpid), fgrcenter=fp_grasp_center,
                                   ...
                                   handoverposid=fpid,
                                   identity=armname + str(fp_gid),
                                   manipulability=manipulability
                                   )
                if armname + str(fp_gid) not in globalidsedges:  # {armname+fpgid: }
                    globalidsedges[armname + str(fp_gid)] = []
                globalidsedges[armname + str(fp_gid)].append('ho' + armname + str(fp_gid) + 'pos' + str(fpid))
    ...
    def bridgeGraph(self):
        for posid, objrotmat4 in enumerate(self.fp_list):
            pass
            if posid not in self.feasible_fp_grasp_pairs:
                continue
            for rgtid, lftid in self.feasible_fp_grasp_pairs[posid]:
    ...
    def __addend(self, objposmat, cond="startrgt", worldframevec3=None, tempobstacle=[]):
        ...
        if "rgt" in cond:
            grasps = self.grasp_list_rgt
        else:
            grasps = self.grasp_list_lft
        ...
        for graspid, graspinfo in enumerate(grasps):
            ...
            ishndcollided = self.hmstr.checkhndenvcollision(ttgsrotmat, self.obstacle_list + tempobstacle,
                                                            armname=cond[-3:], debug=self.debug)
            if not ishndcollided:
                ttgsfgrcenternp = rm.homotransformpoint(objposmat, graspinfo[1])
                ttgsfgrcenternp_handa = ttgsfgrcenternp + handa * self.ret_dis
    ...
    def updateshortestpath(self):
        ...
        # startrgt goalrgt
        if len(self.start_rgt_node_ids) > 0 and len(self.goal_rgt_node_ids) > 0:
            print("Number of start grasps: ", len(self.start_rgt_node_ids), "; Number of goal grasps: ",
        ...
        # startrgt goallft
        if len(self.start_rgt_node_ids) > 0 and len(self.goal_lft_node_ids) > 0:
            print("Number of start grasps: ", len(self.start_rgt_node_ids), "; Number of goal grasps: ",
                  len(self.goal_lft_node_ids))
            startgrip = self.start_rgt_node_ids[0]
        ...
        for i in range(len(pathnidlist) - 1):
            if i == 0 and len(pathnidlist) == 2:
                ...
                if self.regg.edges[pathnidlist[i], pathnidlist[i + 1]]['edgetype'] == "handovertransit":
                    ...
                    objmat40 = self.hndover_pos[self.regg.nodes[nid0]['handoverposid']]
                    ...
                    objmat41 = self.hndover_pos[self.regg.nodes[nid1]['handoverposid']]
...
if __name__ == "__main__":
    ...
    SequenceMaster(get_rbt_by_name("ur3e"),
