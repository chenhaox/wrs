...
if __name__ == '__main__':
    ...
    grasp_info_list = gpa.plan_grasps(gripper_s, obj_cm,
                                      ...
                                      max_samples=16, min_dist_between_sampled_contact_points=.005,
                                      contact_offset=.005)
    obj_name_no_ext = obj_name.split('.')[0]
    gpa.write_pickle_file(obj_name_no_ext, grasp_info_list, root=str(fs.workdir_data),
                          file_name='robotiqhe_grasps.pickle')
    visualize_grasps(grasp_info_list, gripper_s, base, toggle_show_all=False)
    base.run()
