...
def rayhitmesh_drawpath_ss(obj_item, drawpath, direction=np.asarray((0, 0, 1)), toggledebug=False):
    ...
    for i, p in enumerate(drawpath):
        ...
        # except:
        #     continue
        pos, nrml = rayhitmesh_p(obj_item.obj_cm, obj_item.drawcenter, p, direction=direction)
        pos_nrml_list.append([pos, nrml])
        if toggledebug:
            if i == 1:
                fig = plt.figure()
...
def rayhitmesh_drawpath_ms(obj_item, drawpath_ms, direction=np.asarray((0, 0, 1)), error_method='ED'):
    ...
    for drawpath in drawpath_ms:
        ...
        for point in drawpath:
            try:
                pos, nrml = rayhitmesh_p(obj_item.obj_cm, obj_item.drawcenter, point, direction=direction)
                pos_nrml_list.append([pos, nrml])
            except:
...
def prj_drawpath_ss_on_pcd(obj_item, drawpath, mode='DI', direction=np.asarray((0, 0, 1)), error_method='ED',
    ...
    pos_nrml_list, error, error_list, time_cost = \
        __prj_stroke(drawpath, obj_item.drawcenter, obj_item.pcd, obj_item.nrmls, kdt_d3, objcm=obj_item.obj_cm,
    ...
    for i in range(0, len(drawpath), step):
        ...
        pos_nrml_list, error, error_list, time_cost = \
            __prj_stroke(drawpath_tmp, obj_item.drawcenter, obj_item.pcd, obj_item.nrmls, kdt_d3,
                         objcm=obj_item.obj_cm, mode=mode, pcd_start_p=None, pcd_start_n=None, direction=direction,
...
def prj_drawpath_ss_SI_loop(obj_item, drawpath, error_method='ED', toggledebug=False, step=1):
    ...
    for i in range(0, len(drawpath), step):
        ...
        drawpath_tmp = drawpath[i:] + drawpath[:i]
        time_start = time.time()
        uvs, vs, nrmls, faces, avg_scale = cu.lscm_objcm(obj_item.obj_cm, toggledebug=toggledebug)
        ...
        if i > 0:
            ...
            gotostart_pos_nrml_list, _, _, time_cost = \
                __prj_stroke(gotostart_stroke, obj_item.drawcenter, obj_item.pcd, obj_item.nrmls, kdt_d3,
                             objcm=obj_item.obj_cm, mode=mode, direction=direction,
            ...
            stroke_pos_nrml_list, error, error_list, time_cost = \
                __prj_stroke(stroke, obj_item.drawcenter, obj_item.pcd, obj_item.nrmls, kdt_d3,
                             objcm=obj_item.obj_cm, mode=mode, error_method=error_method, direction=direction,
            ...
            stroke_pos_nrml_list, error, error_list, time_cost = \
                __prj_stroke(stroke, obj_item.drawcenter, obj_item.pcd, obj_item.nrmls, kdt_d3,
                             objcm=obj_item.obj_cm, mode=mode, direction=direction, error_method=error_method,
                             toggledebug=toggledebug, surface=surface, transmat=transmat)
            time_cost_total += time_cost
...
def prj_drawpath_ms_SI(obj_item, drawpath_ms, toggledebug=False):
    time_cost_total = 0
    uvs, vs, nrmls, faces, scale_list = cu.lscm_objcm(obj_item.obj_cm, toggledebug=toggledebug)
...
def prj_drawpat_ms_II_temp(obj_item, drawpath_ms, sample_num=None, toggledebug=False):
    ...
    uvs, vs, nrmls, faces, scale_list = \
        cu.lscm_parametrization_objcm_temp(obj_item.obj_cm, toggledebug=toggledebug, sample_num=sample_num)
    uv_center = cu.get_uv_center(uvs)
