...
class OpenArmGripper(gpi.GripperInterface):
    ...
    def __init__(self,
        ...
        self.jlc.anchor.lnk_list[0].cmodel = mcm.CollisionModel(
            initor=os.path.join(current_file_dir, "meshes", "gripper_base_final.stl"),
            name="openarm_gripper_base",
        ...
        # self.jlc.jnts[0].loc_pos = np.array([0, -0.006 ,0.015])
        ...
        self.jlc.jnts[0].motion_range = np.array([0.0, 0.044])
        self.jlc.jnts[0].lnk.cmodel = mcm.CollisionModel(
            initor=os.path.join(current_file_dir, "meshes", "finger_final_left.stl"),
            name="openarm_gripper_left_finger",
            cdmesh_type=self.cdmesh_type)
        self.jlc.jnts[0].lnk.cmodel.rgba = np.array([1, 0, 0, 1])


        self.jlc.jnts[1].change_type(rkjlc.const.JntType.PRISMATIC, np.array([0,self.jaw_range[1]/2]))
        self.jlc.jnts[1].loc_pos = np.array([0, 0.0, 0.])
        self.jlc.jnts[1].loc_rotmat = rm.rotmat_from_euler(0, 0, 0)
        self.jlc.jnts[1].loc_motion_ax = -rm.const.y_ax
        self.jlc.jnts[1].motion_range = np.array([0.0, 0.044])
        self.jlc.jnts[1].lnk.cmodel = mcm.CollisionModel(
            initor=os.path.join(current_file_dir, "meshes", "finger_final_right.stl"),
            name="openarm_gripper_right_finger",
            cdmesh_type=self.cdmesh_type)
        self.jlc.jnts[1].lnk.cmodel.rgba = np.array([0, 0, 1, 1])

        self.jlc.finalize()
        self.loc_acting_center_pos = np.array([0, 0, 0.16])
...
if __name__ == '__main__':
    ...
    # base.run()
    grpr = OpenArmGripper(cdmesh_type=mcm.const.CDMeshType.OBB)
    ...
    grpr.change_jaw_width(.088)
    print(f"当前夹爪开口宽度: {grpr.get_jaw_width():.3f} m")
    grpr.gen_meshmodel(toggle_tcp_frame=True, toggle_jnt_frames=False, toggle_cdprim=False,alpha=.3).attach_to(base)
    base.run()