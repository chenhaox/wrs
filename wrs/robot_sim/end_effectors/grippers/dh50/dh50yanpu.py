...
class Dh50(gp.GripperInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), coupling_offset_pos=np.zeros(3),
                 coupling_offset_rotmat=np.eye(3), cdmesh_type='convex_hull', name='Dh60',enable_cc=True):
        ...
        self.lft.jnts[0]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, math.pi/2)
        self.lft.lnks[0]['mesh_file'] = os.path.join(this_dir, "meshes", "base.stl")
        self.lft.lnks[0]['rgba'] = [.2, .2, .2, 1]

        self.lft.jnts[1]['loc_pos'] = np.array([.00527, -.01315, .1063])
        ...
        self.lft.lnks[1]['mesh_file'] = os.path.join(this_dir, "meshes", "lf1.stl")
        self.lft.lnks[1]['rgba'] = [.5, .5, .5, 1]
        self.lft.jnts[2]['loc_pos'] = np.array([.0294, .01335, -.007])
        ...
        self.rgt.jnts[1]['loc_pos'] = np.array([-.00527, .01315, .1063])
        self.rgt.jnts[1]['type'] = 'prismatic'
        self.rgt.jnts[1]['loc_motionax'] = np.array([-1, 0, 0])
        self.rgt.lnks[1]['name'] = "finger2"
        self.rgt.lnks[1]['mesh_file'] = os.path.join(this_dir, "meshes", "rg1.stl")
        ...
        # jaw center
        self.jaw_center_pos = np.array([0, 0, .16882]) + coupling_offset_pos
        # reinitialize
    ...
    def fix_to(self, pos, rotmat):
        ...
        self.cpl_end_pos = self.coupling.jnts[-1]['gl_posq']
        self.cpl_end_rotmat = self.coupling.jnts[-1]['gl_rotmatq']
        self.lft.fix_to(self.cpl_end_pos, self.cpl_end_rotmat)
        self.lft.fix_to(self.cpl_end_pos, self.cpl_end_rotmat)
        self.rgt.fix_to(self.cpl_end_pos, self.cpl_end_rotmat)
    ...
    def get_pos_rot(self):
        return self.jaw_center_pos, self.coupling.jnts[-1]['gl_rotmatq']
...
if __name__ == '__main__':
    ...
    pos , rot = grpr.get_pos_rot()
    gm.gen_sphere(pos=pos,radius=.01,rgba=[1,0,0,1]).attach_to(base)
    # base.run()

    # grpr.open()
    # grpr.close()
    grpr.show_cdprimit()
    # grpr.gen_meshmodel(                      toggle_tcpcs=True,
