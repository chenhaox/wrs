...
ASSEMBLY_ENV_VERSION = "2026-07-17-v4-exact-geometry-l15"
...
from .fixed_l2_validator import (
    FixedL2Result,
    FixedL2Validator,
...
from .grasp_hint_cache import GraspHintCache
from .ik_hint_cache import IKHintCache
...
class AssemblyGridState:
    ...
    def _add_preassembled_first_part(self) -> None:
        ...
        # Keep the strict searcher's collision model synchronized with the
        # preassembled object. L1.5 exact geometry checks reuse the same
        # staging collision models and helper functions as strict L2.
        if first in self.searcher.staging_models:
            cm = self.searcher.staging_models[first]
            cm.pos = np.asarray(goal_pos, dtype=float).copy()
            cm.rotmat = np.asarray(goal_rot, dtype=float).copy()
    ...
    def _sync_searcher_staging_pose(
        self,
        part_id: str,
        pose_id: int,
        xy: Sequence[float],
    ) -> None:
        """Synchronize one accepted grid placement into strict-L2 models."""
        cand = self._candidate(str(part_id), int(pose_id))
        self.searcher._apply_staging_pose(
            str(part_id),
            np.asarray(xy, dtype=float),
            cand,
        )

    def _sync_all_committed_staging_poses(self) -> None:
        """Synchronize the preassembled part and all accepted decisions."""
        first = self.task.first_part
        if first in self.searcher.staging_models and first in self.searcher.world_poses:
            goal_pos, goal_rot = self.searcher.world_poses[first]
            cm = self.searcher.staging_models[first]
            cm.pos = np.asarray(goal_pos, dtype=float).copy()
            cm.rotmat = np.asarray(goal_rot, dtype=float).copy()

        for part_id, pose_id in self.selected_pose_ids.items():
            if part_id not in self.selected_xy:
                continue
            self._sync_searcher_staging_pose(
                part_id,
                int(pose_id),
                self.selected_xy[part_id],
            )

    def exact_geometry_check(self, action: int) -> PlacementCheck:
        """L1.5 selected-action exact geometry check.

        The strict L2 searcher is reused for exact collision, mesh clearance,
        robot-home collision and robot-home clearance checks. The candidate
        pose is temporary and all collision models are restored before return.
        """
        base_check = self.check_action(int(action))
        ...
        part_id = str(self.current_part)
        cand = self._candidate(part_id, pose_id)
        xy = self.workspace.spec.cell_center(row, col)

        active_parts: List[str] = []
        first = self.task.first_part
        if first in self.searcher.staging_models:
            active_parts.append(first)
        for pid in self.selected_pose_ids:
            if pid in self.searcher.staging_models and pid not in active_parts:
                active_parts.append(pid)
        if part_id in self.searcher.staging_models and part_id not in active_parts:
            active_parts.append(part_id)

        snapshots: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
        for pid in active_parts:
            cm = self.searcher.staging_models[pid]
            snapshots[pid] = (
                np.asarray(cm.pos, dtype=float).copy(),
                np.asarray(cm.rotmat, dtype=float).copy(),
            )

        try:
            self._sync_all_committed_staging_poses()
            self.searcher._apply_staging_pose(part_id, xy, cand)

            collision_hit = self.searcher._pairwise_collision(
                active_pids=active_parts
            )
            if collision_hit:
                return PlacementCheck(
                    False,
                    "exact_pair_collision",
                    blocker=str(collision_hit),
                )

            clearance_hit = self.searcher._mesh_clearance_reason(
                active_pids=active_parts
            )
            if clearance_hit:
                return PlacementCheck(
                    False,
                    "exact_mesh_clearance",
                    blocker=str(clearance_hit),
                )

            home_hit = self.searcher._robot_home_collision_reason(
                active_pids=[part_id]
            )
            if home_hit:
                return PlacementCheck(
                    False,
                    "exact_robot_home_collision",
                    blocker=str(home_hit),
                )

            home_clearance_hit = self.searcher._robot_home_clearance_reason(
                active_pids=[part_id]
            )
            if home_clearance_hit:
                return PlacementCheck(
                    False,
                    "exact_robot_home_clearance",
                    blocker=str(home_clearance_hit),
                )
            ...
            return PlacementCheck(
                False,
                "exact_geometry_exception",
                blocker=f"{type(exc).__name__}: {exc}",
            )
        finally:
            for pid, (old_pos, old_rotmat) in snapshots.items():
                cm = self.searcher.staging_models[pid]
                cm.pos = old_pos
                cm.rotmat = old_rotmat
    ...
    def apply_action(self, action: int) -> FootprintRecord:
        ...
        # Commit the accepted action into the strict searcher's collision model
        # so the next L1.5 check sees all already placed parts.
        self._sync_searcher_staging_pose(part, pose_id, xy)
...
class AssemblyLayoutEnv(_EnvBase):
    ...
    def __init__(
        ...
        *,
        ...
        grasp_hint_path: Optional[str] = None,
        grasp_reward_weight: float = 0.10,
        mask_invalid_grasp_poses: bool = False,
        ik_hint_path: Optional[str] = None,
        ik_reward_weight: float = 0.20,
        mask_zero_ik_actions: bool = False,
        ik_mask_threshold: float = 0.0,
        exact_geometry_check: bool = True,
        exact_geometry_reject_penalty: float = -1.0,
    ) -> None:
        ...
        self.grasp_reward_weight = float(grasp_reward_weight)
        if self.grasp_reward_weight < 0.0:
            raise ValueError("grasp_reward_weight must be non-negative")
        self.mask_invalid_grasp_poses = bool(mask_invalid_grasp_poses)
        self.grasp_hint_cache: Optional[GraspHintCache] = None
        if grasp_hint_path:
            self.grasp_hint_cache = GraspHintCache.load(
                grasp_hint_path,
                decision_parts=task.decision_parts,
                max_poses=self.max_poses,
            )
        self.ik_reward_weight = float(ik_reward_weight)
        if self.ik_reward_weight < 0.0:
            raise ValueError("ik_reward_weight must be non-negative")
        self.mask_zero_ik_actions = bool(mask_zero_ik_actions)
        self.ik_mask_threshold = float(ik_mask_threshold)
        if not 0.0 <= self.ik_mask_threshold <= 1.0:
            raise ValueError("ik_mask_threshold must be in [0,1]")
        self.exact_geometry_check = bool(exact_geometry_check)
        self.exact_geometry_reject_penalty = float(
            exact_geometry_reject_penalty
        )
        if not math.isfinite(self.exact_geometry_reject_penalty):
            raise ValueError("exact_geometry_reject_penalty must be finite")
        self._ik_hint_path = str(ik_hint_path) if ik_hint_path else None
        self.ik_hint_cache: Optional[IKHintCache] = None
        ...
        if self._ik_hint_path:
            self.ik_hint_cache = IKHintCache.load(
                self._ik_hint_path,
                decision_parts=task.decision_parts,
                max_poses=self.max_poses,
            ...
            missing_regions = [
                rid for rid in self.region_ids if not self.ik_hint_cache.has_region(rid)
            ]
            if self.region_mode == REGION_MODE_FIXED:
                missing_regions = [
                    self.fixed_region_id
                ] if not self.ik_hint_cache.has_region(self.fixed_region_id) else []
            if missing_regions:
                raise ValueError(
                    "IK hint cache does not cover environment regions: "
                    f"{missing_regions}; cache={self.ik_hint_cache.region_ids}"
                )
    ...
    def step(
        ...
        action = int(action)
        env_mask = self.action_masks()
        if action < 0 or action >= self.action_n or not bool(env_mask[action]):
            check = PlacementCheck(False, "action_mask_rejected")
        else:
        ...
        grasp_hint_value = self._current_grasp_hint_value(part, pose_id)
        ik_hint_value = self._current_ik_hint_value(part, pose_id, row, col)

        # L1.5: validate only the selected action with strict 3-D geometry
        # before mutating the grid state.
        if self.exact_geometry_check:
            exact_check = state.exact_geometry_check(action)
            if not exact_check.valid:
                self._episode_steps += 1
                self._terminated = True
                reward = float(self.exact_geometry_reject_penalty)
                self._episode_return += reward
                info = self._base_info()
                info.update(
                    {
                        "event": "exact_geometry_rejected",
                        "action": action,
                        "part_id": part,
                        "pose_id": int(pose_id),
                        "grid_row": int(row),
                        "grid_col": int(col),
                        "xy": state.workspace.spec.cell_center(row, col).tolist(),
                        "grasp_hint": float(grasp_hint_value),
                        "ik_hint": float(ik_hint_value),
                        "failure_reason": exact_check.reason,
                        "failure_blocker": exact_check.blocker,
        ...
        self._episode_steps += 1
        distance_reward = self._placement_shaping_reward(part, record.xy)
        grasp_reward = self.grasp_reward_weight * grasp_hint_value
        ik_reward = self.ik_reward_weight * ik_hint_value
        reward = distance_reward + grasp_reward + ik_reward
        info = self._base_info()
        info.update(
            {
                "event": "placement",
        ...
        if state.done:
            ...
            self._terminated = True
        elif not np.any(self.action_masks()):
            reward -= 2.0
            self._terminated = True
            info.update(
                {
    ...
    def action_masks(self) -> np.ndarray:
        ...
        mask = self.state.flat_action_mask().astype(bool, copy=True)
        if (
            self.mask_invalid_grasp_poses
            and self.grasp_hint_cache is not None
            and self.state.current_part is not None
        ):
            pose_valid = self.grasp_hint_cache.valid_pose_mask(
                str(self.state.current_part), self.max_poses
            )
            cells = self.grid_height * self.grid_width
            for pose_id, valid in enumerate(pose_valid):
                if not valid:
                    start = pose_id * cells
                    mask[start : start + cells] = False
        if (
            self.mask_zero_ik_actions
            and self.ik_hint_cache is not None
            and self.state.current_part is not None
            and self.current_region_id is not None
        ):
            ik_mask = self.ik_hint_cache.positive_action_mask(
                str(self.current_region_id),
                str(self.state.current_part),
                threshold=self.ik_mask_threshold,
            ).reshape(-1)
            mask &= ik_mask
        return mask
    ...
    def check_exact_action(self, action: int) -> PlacementCheck:
        """Public proposal-check API used by evaluate_hint_policy.py."""
        state = self._require_state()
        if self._terminated or self._truncated:
            return PlacementCheck(False, "episode_already_complete")
        return state.exact_geometry_check(int(action))
    ...
    def _observation(self) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            "pose_mask": pose_mask,
            "grasp_hint": self._current_grasp_hint(state),
            "ik_hint": self._current_ik_hint(state),
            "hint_available": np.asarray(
                [
                    1 if self.grasp_hint_cache is not None else 0,
                    1 if self.ik_hint_cache is not None else 0,
                ],
                dtype=np.int8,
            ),
        }

    def _current_grasp_hint(self, state: AssemblyGridState) -> np.ndarray:
        if (
            self.grasp_hint_cache is None
            or self._terminated
            or state.current_part is None
        ):
            return np.zeros(self.max_poses, dtype=np.float32)
        return self.grasp_hint_cache.vector(str(state.current_part), self.max_poses)

    def _current_grasp_hint_value(self, part: str, pose_id: int) -> float:
        if self.grasp_hint_cache is None:
            return 0.0
        vector = self.grasp_hint_cache.vector(str(part), self.max_poses)
        if pose_id < 0 or pose_id >= vector.size:
            return 0.0
        return float(vector[pose_id])

    def _current_ik_hint(self, state: AssemblyGridState) -> np.ndarray:
        if (
            self.ik_hint_cache is None
            or self._terminated
            or state.current_part is None
            or self.current_region_id is None
        ):
            return np.zeros(
                (self.max_poses, self.grid_height, self.grid_width),
                dtype=np.float32,
            )
        return self.ik_hint_cache.map(
            str(self.current_region_id), str(state.current_part)
        )

    def _current_ik_hint_value(
        self, part: str, pose_id: int, row: int, col: int
    ) -> float:
        if self.ik_hint_cache is None or self.current_region_id is None:
            return 0.0
        return self.ik_hint_cache.value(
            str(self.current_region_id), str(part), pose_id, row, col
        )
    ...
    def compact_description(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "grasp_hint_available": self.grasp_hint_cache is not None,
            "grasp_hint_path": (
                self.grasp_hint_cache.source_path if self.grasp_hint_cache else None
            ),
            "grasp_reward_weight": self.grasp_reward_weight,
            "mask_invalid_grasp_poses": self.mask_invalid_grasp_poses,
            "ik_hint_available": self.ik_hint_cache is not None,
            "ik_hint_path": (
                self.ik_hint_cache.source_path if self.ik_hint_cache else None
            ),
            "ik_reward_weight": self.ik_reward_weight,
            "mask_zero_ik_actions": self.mask_zero_ik_actions,
            "ik_mask_threshold": self.ik_mask_threshold,
            "exact_geometry_check": self.exact_geometry_check,
            "exact_geometry_reject_penalty": self.exact_geometry_reject_penalty,
            "observation_shapes": {
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--grasp-hint",
        default=None,
        help="NPZ generated by precompute_grasp_hint.py.",
    )
    parser.add_argument("--grasp-reward-weight", type=float, default=0.10)
    parser.add_argument(
        "--mask-invalid-grasp-poses",
        action="store_true",
        help=(
            "Hard-mask poses whose grasp cache has no action centre above the "
            "table. Keep disabled for the first cache inspection run."
        ),
    )
    parser.add_argument(
        "--ik-hint",
        default=None,
        help="NPZ generated by precompute_ik_hint.py.",
    )
    parser.add_argument("--ik-reward-weight", type=float, default=0.20)
    parser.add_argument(
        "--mask-zero-ik-actions",
        action="store_true",
        help="Hard-mask actions with IK hint <= --ik-mask-threshold.",
    )
    parser.add_argument("--ik-mask-threshold", type=float, default=0.0)
    parser.add_argument(
        "--no-exact-geometry-check",
        action="store_true",
        help=(
            "Disable selected-action L1.5 exact 3-D collision, mesh-clearance "
            "and robot-home checks. Enabled by default."
        ),
    )
    parser.add_argument(
        "--exact-geometry-reject-penalty",
        type=float,
        default=-1.0,
    )
...
def main() -> None:
    print(f"[assembly_layout_env] version={ASSEMBLY_ENV_VERSION}")
    args = _parse_args()
    ...
    env = AssemblyLayoutEnv(
        ...
        exact_geometry_check=not args.no_exact_geometry_check,
        ...
        for episode in range(args.episodes):
            ...
            while not (terminated or truncated):
                ...
                print(
                    ...
                    f"g_hint={float(final_info.get('grasp_hint', 0.0)):.3f} "
                    f"ik_hint={float(final_info.get('ik_hint', 0.0)):.3f} "
