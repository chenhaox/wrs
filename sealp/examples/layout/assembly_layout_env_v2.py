...
ASSEMBLY_ENV_VERSION = "2026-07-16-v2-grasp-hint"
...
from sealp.examples.layout.assembly_rl.grasp_hint_cache import GraspHintCache  # noqa: E402
...
class AssemblyLayoutEnv(_EnvBase):
    ...
    def _observation(self) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            # IK hint is added in the next project step.
            "ik_hint": np.zeros(
                (self.max_poses, self.grid_height, self.grid_width),
                dtype=np.float32,
            ),
            "hint_available": np.asarray(
                [1 if self.grasp_hint_cache is not None else 0, 0],
                dtype=np.int8,
            ),
        }
