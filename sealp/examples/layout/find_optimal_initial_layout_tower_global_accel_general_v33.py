#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""General deterministic accelerated Global-to-Local layout search V3.3.
...
import uniform_grasp_accel_v33 as uniform_accel
...
ACFG: Dict[str, object] = {
    ...
    # Fast search may use feature-space FPS; final 350-grasp scoring defaults
    # to the historical np.linspace index subset for score comparability.
    "search_subset_mode": "fps",
    "authoritative_subset_mode": "legacy_uniform",
...
class GlobalAccelSearcher(gmod.GlobalLayoutSearcher):
    ...
    def _candidate_record(
        ...
        record = {
            ...
            "grasp_subset_mode": str(
                getattr(cand, "_grasp_subset_mode", "unknown")
            ),
            "evaluated_grasp_cap": int(
                getattr(cand, "_evaluated_grasp_cap", -1)
            ),
    ...
    def _fresh_evaluate(
        ...
        old_subset_mode = uniform_accel.get_subset_mode()

        if int(cap) == 0:
            eval_subset_mode = str(ACFG["authoritative_subset_mode"])
        elif int(cap) == int(ACFG["authoritative_grasp_cap"]):
            eval_subset_mode = str(ACFG["authoritative_subset_mode"])
        else:
            eval_subset_mode = str(ACFG["search_subset_mode"])

        fast.MAX_GRASPS_PER_POSE = int(cap)
        uniform_accel.set_subset_mode(eval_subset_mode)
        try:
    ...
    def _authoritative_rescore(
        ...
        for i, src in enumerate(unique, 1):
            ...
            authoritative_mode = str(
                ACFG["authoritative_subset_mode"]
            )
            if (
                int(getattr(src, "_evaluated_grasp_cap", -1)) == cap
                and str(
                    getattr(src, "_grasp_subset_mode", "")
                ) == authoritative_mode
...
def _consume_accel_args() -> None:
    pairs = {
        ...
        "--global-accel-search-subset-mode": (
            "search_subset_mode",
            str,
        ),
        "--global-accel-authoritative-subset-mode": (
            "authoritative_subset_mode",
            str,
        ),
    ...
    if grasp_mode not in {
        ...
        raise ValueError(
            ...
            "same_grasp_progressive, same_grasp_full, or legacy_fast"
        )
    ...
    for cfg_key, cli_name in (
        ("search_subset_mode", "--global-accel-search-subset-mode"),
        (
            "authoritative_subset_mode",
            "--global-accel-authoritative-subset-mode",
        ),
        ...
        try:
            ACFG[cfg_key] = uniform_accel.normalize_subset_mode(
                str(ACFG[cfg_key])
            )
        except ValueError as exc:
            raise ValueError(
                f"{cli_name} must be fps or legacy_uniform"
            ) from exc
...
def main() -> None:
    ...
    uniform_accel.install(
        ...
        subset_mode=str(ACFG["search_subset_mode"]),
    )
    ...
    print("[global-accel-v3.3] config:")
    for key, value in ACFG.items():
        print(f"    {key:30s} = {value}")
    print("[global-accel-v3.3] original Global file remains untouched")
    print(
        "[global-accel-v3.3] rotation handling: Halton prioritizes "
    ...
    try:
        ...
        print(
            f"[global-accel-v3.3] wall-clock total = "
