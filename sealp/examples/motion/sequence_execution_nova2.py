...
def main():
    ...
    import wrs.robot_sim.robots.nova2_wg.nova2wg3 as nw3
    ...
    # 将椅子稍微推远一点，给四条腿留出空间
    plan.set_staging("seat", pos=np.array([0.35, 0.00, 0.00]))
    plan.set_staging("leg_fl", pos=np.array([0.25, 0.25, 0.00]))
    plan.set_staging("leg_fr", pos=np.array([0.25, -0.25, 0.00]))
    plan.set_staging("leg_bl", pos=np.array([0.35, 0.25, 0.00]))
    plan.set_staging("leg_br", pos=np.array([0.35, -0.25, 0.00]))
    ...
    # 5. Robot
    # ------------------------------------------------------------------
    robot = nw3.Nova2WG3(enable_cc=True)
    ...
    executor = SequenceExecutor(
        ...
        grasp_paths={
            'leg_model': r"D:\Project\wrs-sealp\sealp\examples\grasp\_output\demo_yuanchair-part2_wrs_grasps.pickle",
            'seat_model': r"D:\Project\wrs-sealp\sealp\examples\grasp\_output\demo_yuanchair-part1_wrs_grasps.pickle"
        }
    )
