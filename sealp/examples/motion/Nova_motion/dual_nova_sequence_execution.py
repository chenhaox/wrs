"""
Dual-Arm Nova2 Sequence Execution — YuanChair Assembly
=======================================================

与 ``dual_sequence_execution`` 相同的装配与预搜索逻辑，机器人换为 **双臂 Nova2WG3**：
左臂基座在 ``pos``，右臂基座在 ``pos + [0,-0.6,0]``（与 Piper 双臂布局同型，间距 60 cm）。

Usage::

    python -m sealp.examples.motion.Nova_motion.dual_nova_sequence_execution
"""
...
from wrs.robot_sim.robots.nova2_wg.nova2_dual_arm import DualNova2WG3
...
def main():
    ...
    asmdef_dir = os.path.join(
        os.path.dirname(__file__), "..", "..", "..", "assembly_sequence", "_demo_output")
    ...
    plan = TaskPlan(
        ...
        name="YuanChair Dual Nova2 Execution",
        description="Dual Nova2WG3 sequential assembly of the YuanChair.",
    )
    ...
    robot = DualNova2WG3(enable_cc=True)
    home = np.zeros(6)
